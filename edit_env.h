...
enum
{
	...
	GUI_ID_VIEWPORT_3D_RIGHTCLICK_MENU_ITEM_VIEW_DYNAMIC_LIGHT,
...
class CameraPanel //: public gui::IGUIElement
{
protected:
    scene::ISceneManager* smgr=NULL;
    ...
    core::vector2di windowOrigin;

public:
    CameraPanel(core::dimension2du wsize, core::vector2di origin);

    void Initialize(scene::ISceneManager*,MyEventReceiver*,geometry_scene*);

    bool IsInWindow(core::vector2di coords);
    bool hasControl() {return this->bHasControl;}

    virtual scene::ICameraSceneNode* getCamera() {};
    virtual int Update() {};
    virtual void drawGrid(video::IVideoDriver* driver, const video::SMaterial material) {};
    virtual void drawCircle(video::IVideoDriver* driver, const video::SMaterial material,core::vector2di v,int rad) {};
    virtual void drawAll(video::IVideoDriver* driver) {};
};

class CameraPanel3D : public CameraPanel
{
public:
    CameraPanel3D(core::dimension2du wsize, core::vector2di origin);
    scene::ICameraSceneNode* getCamera();
    int Update();
    void drawAll(video::IVideoDriver* driver);
};

#define CAMERA_X_AXIS 0
#define CAMERA_Y_AXIS 1
#define CAMERA_Z_AXIS 2

class CameraPanel2D : public CameraPanel
{
    ...
    f32 getViewScaling() { return ((f32)this->viewSize.Width / this->windowSize.Width); }

public:
    CameraPanel2D(core::dimension2du wsize, core::vector2di origin);
    scene::ICameraSceneNode* getCamera();
    int Update() {};
    void setAxis(int);
    void drawGrid(video::IVideoDriver* driver, const video::SMaterial material);
    void drawCircle(video::IVideoDriver* driver, const video::SMaterial material,core::vector2di v,int rad,video::SColor col);
    void drawAll(video::IVideoDriver* driver);
    bool click_hits_poly(polyfold*, core::vector2di v);
};
