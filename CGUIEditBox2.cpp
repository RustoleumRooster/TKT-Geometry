...
//! constructor
CGUIEditBox2::CGUIEditBox2(const wchar_t* text, bool border,
...
void CGUIEditBox2::setOverrideFont(IGUIFont* font)
{
...
IGUIFont * CGUIEditBox2::getOverrideFont() const
{
	return OverrideFont;
}

//! Get the font which is used right now for drawing
IGUIFont* CGUIEditBox2::getActiveFont() const
{
	if ( OverrideFont )
		return OverrideFont;
...
void CGUIEditBox2::setOverrideColor(video::SColor color)
{
	OverrideColor = color;
...
video::SColor CGUIEditBox2::getOverrideColor() const
{
...
void CGUIEditBox2::setDrawBorder(bool border)
{
	Border = border;
}

//! Sets whether to draw the background
void CGUIEditBox2::setDrawBackground(bool draw)
{
	Background = draw;
}

//! Sets if the text should use the overide color or the color in the gui skin.
void CGUIEditBox2::enableOverrideColor(bool enable)
{
	OverrideColorEnabled = enable;
}

bool CGUIEditBox2::isOverrideColorEnabled() const
{
...
void CGUIEditBox2::setWordWrap(bool enable)
{
	WordWrap = enable;
	breakText();
}
...
void CGUIEditBox2::updateAbsolutePosition()
{
...
bool CGUIEditBox2::isWordWrapEnabled() const
{
...
void CGUIEditBox2::setMultiLine(bool enable)
{
	MultiLine = enable;
	breakText();
}
...
bool CGUIEditBox2::isMultiLineEnabled() const
{
...
void CGUIEditBox2::setPasswordBox(bool passwordBox, wchar_t passwordChar)
{
...
bool CGUIEditBox2::isPasswordBox() const
{
...
void CGUIEditBox2::setTextAlignment(EGUI_ALIGNMENT horizontal, EGUI_ALIGNMENT vertical)
{
	HAlign = horizontal;
	VAlign = vertical;
}
...
bool CGUIEditBox2::OnEvent(const SEvent& event)
{
	if (isEnabled())
	{
		...
		case EET_GUI_EVENT:
			...
			else if(event.GUIEvent.EventType == gui::EGET_ELEMENT_HOVERED)
            {
                ...
                OverrideColor = skin->getColor(gui::EGDC_HIGH_LIGHT_TEXT);
...
bool CGUIEditBox2::processKey(const SEvent& event)
{
...
void CGUIEditBox2::draw()
{
    ...
    if(hovered)
        col = video::SColor(255,32,48,32);
    else if( focus )
        col = skin->getColor(EGDC_FOCUSED_EDITABLE);
    else
        col = skin->getColor(EGDC_EDITABLE);
	...
	{
		// draw the border
		skin->draw3DSunkenPane(this, col, false, Background, AbsoluteRect, &AbsoluteClippingRect);
...
void CGUIEditBox2::setText(const wchar_t* text)
{
	Text = text;
...
void CGUIEditBox2::setAutoScroll(bool enable)
{
	AutoScroll = enable;
}
...
bool CGUIEditBox2::isAutoScrollEnabled() const
{
...
core::dimension2du CGUIEditBox2::getTextDimension()
{
	core::rect<s32> ret;
...
//! infinity.
void CGUIEditBox2::setMax(u32 max)
{
	Max = max;
...
u32 CGUIEditBox2::getMax() const
{
	return Max;
}


bool CGUIEditBox2::processMouse(const SEvent& event)
{
...
s32 CGUIEditBox2::getCursorPos(s32 x, s32 y)
{
	IGUIFont* font = getActiveFont();
...
void CGUIEditBox2::setTextRect(s32 line)
{
	if ( line < 0 )
		return;
...
s32 CGUIEditBox2::getLineFromPos(s32 pos)
{
	if (!WordWrap && !MultiLine)
		return 0;
...
void CGUIEditBox2::inputChar(wchar_t c)
{
	if (!isEnabled())
		return;
...
void CGUIEditBox2::calculateScrollPos()
{
	if (!AutoScroll)
		return;
...
void CGUIEditBox2::calculateFrameRect()
{
...
//! set text markers
void CGUIEditBox2::setTextMarkers(s32 begin, s32 end)
{
...
void CGUIEditBox2::sendGuiEvent(EGUI_EVENT_TYPE type)
{
	if ( Parent )
	{
		SEvent e;
...
void CGUIEditBox2::serializeAttributes(io::IAttributes* out, io::SAttributeReadWriteOptions* options=0) const
{
...
void CGUIEditBox2::deserializeAttributes(io::IAttributes* in, io::SAttributeReadWriteOptions* options=0)
{
